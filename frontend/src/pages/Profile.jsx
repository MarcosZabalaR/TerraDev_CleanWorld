...
// ID del usuario autenticado (¡DEBES REEMPLAZAR ESTO CON EL ID REAL DEL ESTADO DE AUTENTICACIÓN!)
...
export default function Profile() {
  ...
  // --- Lógica de Cambio de Nombre de Usuario ---
  ...
  const handleUsernameUpdate = async (e) => {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/users/${AUTHENTICATED_USER_ID}`, {
            method: 'PATCH', // Usamos PATCH para actualizar solo el nombre
            headers: {
                'Content-Type': 'application/json',
                // ** IMPORTANTE: Aquí deberías añadir el token de autorización (Bearer Token) **
  ...
  // Función para actualizar la contraseña (FUNCIONAL)
  const handlePasswordUpdate = async (e) => {
    ...
    try {
        ...
        if (!response.ok) {
            // Intenta leer el mensaje de error del backend
            const errorData = await response.json();
            throw new Error(errorData.message || "Error al cambiar la contraseña. Asegúrate que la contraseña actual sea correcta.");
        }
  ...
  // Si el usuario es null después de la carga y hay un error, evitamos renderizar el resto.
  if (!user) {
      return null;
  }
      ...
      <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div className="p-8 -mt-20">
            {/* Contenedor de la Imagen de Perfil y Botón de Cambio */}
            ...
            <p className="text-center text-xl text-brand-primary mb-12">
                Miembro de CleanWorld
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
                ...
                {isEditingUsername && (
                    <form onSubmit={handleUsernameUpdate} className="mt-4 p-4 bg-white border border-yellow-300 rounded-lg shadow-lg">
                        ...
                        <div className="flex justify-end space-x-3">
                            ...
                            <button 
                                type="submit"
                                className="flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-brand-primary hover:bg-brand-dark transition duration-150 disabled:opacity-50"
                                disabled={loading}
                            >
                                {loading ? <IconLoader2 size={20} className="animate-spin mr-1" /> : 'Guardar'}
                            </button>
                        </div>
                    </form>
                )}
              ...
              <div className="col-span-1 md:col-span-2 mt-6">
                ...
                {!isChangingPassword && (
                    <button 
                        onClick={() => { setIsChangingPassword(true); setIsEditingUsername(false); setPasswordError(null); }}
                        className="flex justify-between items-center w-full px-6 py-3 bg-red-100 text-red-700 rounded-xl font-semibold hover:bg-red-200 transition duration-150"
                        disabled={loading}
                    >
                        Cambiar Contraseña
                ...
                {isChangingPassword && (
                    <form onSubmit={handlePasswordUpdate} className="mt-4 p-4 bg-white border border-red-300 rounded-lg shadow-lg">
                        ...
                        <div className="flex justify-end space-x-3">
                            ...
                            <button 
                                type="submit"
                                className="flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-red-600 hover:bg-red-700 transition duration-150 disabled:opacity-50"
                                disabled={loading}
                            >
                                {loading ? <IconLoader2 size={20} className="animate-spin mr-1" /> : <IconLock size={18} className="mr-1" />}
