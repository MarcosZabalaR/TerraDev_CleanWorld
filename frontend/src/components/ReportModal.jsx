...
export default function ReportModal({ isReportMode, reportCoords, pinPosition = { x: 50, y: 35 }, onClose, onSubmit }) {
  ...
  const severityOptions = [
    { 
      value: 1, 
      label: t('eventModal.mild'), 
      description: t('reportModal.severityDescription.Mild'),
    ...
    { 
      value: 2, 
      label: t('eventModal.moderate'), 
      description: t('reportModal.severityDescription.Moderate'),
    ...
    { 
      value: 3, 
      label: t('eventModal.severe'), 
      description: t('reportModal.severityDescription.Severe'),
      bgColor: 'bg-red-50',
  ...
  const handleImageUpload = (e) => {
    ...
    if (!file || images.length >= 1) return images.length >= 1 && alert(t('reportModal.maxImage'));
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (images.length === 0) {
      alert(t('reportModal.image'));
      return;
    }
    if (!formData.description.trim()) {
      alert(t('reportModal.descriptionProblem'));
      return;
    }
    setIsUploading(true);
    try {
      ...
      const { data } = await axios.post('http://localhost:8080/zones', {
        ...
        status: t('reportModal.dirty'), reported_id: 1
      });
      onSubmit(data);
      resetForm();
    } catch (error) {
      ...
      <div className="fixed top-24 left-1/2 -translate-x-1/2 z-9999 bg-white px-6 py-4 rounded-xl shadow-2xl border-2 border-brand-primary">
        <div className="flex items-center gap-3 text-brand-dark">
          ...
          <div className="flex-1">
            <p className="font-semibold text-base">{t('reportModal.location')}</p>
            <p className="text-sm text-gray-600">{t('reportModal.markMap')}</p>
          </div>
          <button
            onClick={onClose}
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="flex flex-col min-h-0 flex-1">
            <div className="overflow-y-auto p-6 space-y-6">
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">
                  {t('reportModal.title')} <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  name="title"
                  ...
                  required
                  placeholder={t('reportModal.exampleTitle')}
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">
                  {t('reportModal.description')} <span className="text-red-500">*</span>
                </label>
                <textarea
                  name="description"
                  ...
                  required
                  rows="4"
                  placeholder={t('reportModal.exampleDescription')}
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">
                  {t('reportModal.photo')} <span className="text-red-500 font-normal">*</span>
                </label>
                ...
                {images.length > 0 ? (
                  ...
                  <label className="flex flex-col items-center justify-center w-full h-36 border-2 border-dashed border-gray-300 rounded-xl cursor-pointer hover:border-brand-primary hover:bg-brand-light/10 transition-all group">
                    ...
                    <span className="text-sm font-semibold text-gray-700 group-hover:text-brand-primary transition-colors">{t('reportModal.addPhoto')}</span>
            ...
            <div className="p-5 border-t border-gray-200 bg-gray-50 shrink-0 flex gap-3">
              ...
              >
                {isUploading ? 'Subiendo imagen...' : images.length === 0 ? t('reportModal.image') : !formData.description.trim() ? t('reportModal.descriptionProblem') : 'Enviar reporte'}
              </button>
            </div>
          </form>
        </div>
